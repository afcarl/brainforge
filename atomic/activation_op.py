...
class ActivationFunction:
    ...
    def __str__(self):
        return self.type if self.type else self.__class__.__name__.lower()
...
class SoftMax(ActivationFunction):
    ...
    def true_backward(A: np.ndarray):
        ...
        identity = np.eye(A.shape[1], dtype=floatX)
        idx, idy = np.diag_indices(identity)
        return A * (A[..., None] - identity[None, ...])[:, idx, idy]
